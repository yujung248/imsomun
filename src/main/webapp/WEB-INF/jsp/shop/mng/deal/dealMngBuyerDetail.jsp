...
<script type="text/javascript">
	function dealMngBuyerCancel() {
		if (confirm(" 거래 취소 하시려면 예를 누르시고\n거래 취소 하지 않으시려면 아니오를 눌러주세요")) {
			location.href = "/shop/mng/deal/dealMngBuyerCancel.do?d_idx=${dealMngVO.d_idx}";
		} else {
			alert('아니오를 누르셨습니다');
		}
	}

	function dealMngBuyerComplete() {
		if (confirm(" 거래 완료 하시려면 예를 누르시고\n거래 완료를 하지 않으시려면 아니오를 눌러주세요")) {
			location.href = "/shop/mng/deal/dealMngBuyerComplete.do?d_idx=${dealMngVO.d_idx}";
		} else {
			alert('아니오를 누르셨습니다');
		}
	}
	...
	function dealMngBuyerReview() {
		if (confirm(" 후기 작성 하시려면 예를 누르시고\n후기 작성 하지 않으시려면 아니오를 눌러주세요")) {
			location.href = "/shop/user/review/EgovBuyerInsertForm.do?sns_idx=${dealMngVO.sns_idx }&p_idx=${dealMngVO.p_idx}";
		} else {
			alert('아니오를 누르셨습니다');
		}
	}
			...
			<div class="row">
				...
				<table class="table">
					...
					<tr>
						...
						<td>${dealMngVO.s_phone }</td>
					</tr>
					<tr>
						<th style="text-align: right">판매자 계좌</th>
						<td> ${dealMngVO.s_account_n} ${dealMngVO.s_account}</td>						
					</tr>
					<tr>
						...
						<td colspan="2">
							<div style="text-align: right;">
										<button type="button" class="btn btn-danger"
											onclick="dealMngBuyerCancel()">거래 취소</button>
										<button type="button" class="btn btn-success"
											onclick="dealMngBuyerComplete()">거래 완료</button>
										<button type="button" class="btn btn-info"
											onclick="dealMngBuyerReview()">후기 작성</button>
							</div><p>
